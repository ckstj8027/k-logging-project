...
- **No-Offset 페이징 적용:** 수만 건의 데이터가 쌓일수록 느려지는 기존 페이징의 한계를 극복하기 위해 Querydsl을 활용한 Cursor(No-offset) 방식의 페이징을 도입하여 조회 응답 속도를 100ms 이내로 고정시켰습니다.
- **도메인 엔티티 공통화:** 흩어져 있던 공통 필드(Tenant, createdAt 등)를 `@MappedSuperclass` 기반의 부모 클래스(`BaseResourceProfile`)로 추상화하여 중복 코드를 제거하고 무결성을 높였습니다.
