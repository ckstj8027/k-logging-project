<script lang="ts">
  ...
  import type { DeploymentDto, DashboardSummary } from '$lib/types';
  ...
  async function fetchDeployments(lastId?: number) {
    if (loading) return;
    loading = true;
    try {
      const response = await api.get('/assets/deployments', {
        params: {
          lastId,
          size: pageSize
        }
      ...
      const newDeps = response.data;
      if (newDeps.length < pageSize) {
        hasMore = false;
      }
      
      if (lastId) {
        deployments = [...deployments, ...newDeps];
      } else {
        deployments = newDeps;
      }
    } catch (err) {
...
<div class="space-y-8 animate-in fade-in duration-500">
  <div class="flex items-center justify-between">
    <div class="space-y-1">
      <h1 class="text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white uppercase tracking-tighter">Deployments</h1>
      <p class="text-slate-500 dark:text-slate-400 font-medium">Manage and monitor cluster application deployments.</p>
    </div>
  </div>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="bg-slate-50/80 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800">
                ...
                <th class="h-14 px-6 text-left align-middle font-bold text-slate-500 uppercase tracking-widest text-[10px]">Replicas (Avail/Total)</th>
                <th class="h-14 px-6 text-left align-middle font-bold text-slate-500 uppercase tracking-widest text-[10px]">Strategy</th>
              ...
              {#each deployments as dep (dep.id)}
                <tr class="transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-900/30 group">
                  ...
                  <td class="px-6 py-4 align-middle tabular-nums whitespace-nowrap">
                    <span class="text-slate-500 font-medium">{formatDate(dep.createdAt)}</span>
                  </td>
                </tr>
              {/each}
            </tbody>
        ...
        {#if summary}
          <div class="flex flex-col items-center gap-2">
            <div class="flex justify-between w-full max-w-xs text-[10px] font-black uppercase tracking-widest text-slate-400">
              <span>Progress</span>
              <span>{deployments.length} / {summary.deploymentCount} items</span>
            </div>
            <div class="w-full max-w-xs h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
              <div 
                class="h-full bg-primary transition-all duration-500 ease-out" 
                style="width: {(deployments.length / summary.deploymentCount) * 100}%"
              ></div>
            </div>
          </div>
        {/if}
  ...
  {:else if !error && !loading}
    <div class="flex flex-col items-center justify-center h-64 border-2 border-dashed rounded-2xl bg-slate-50/50 dark:bg-slate-900/30 border-slate-200 dark:border-slate-800">
      <p class="text-sm text-slate-400 font-bold uppercase tracking-widest">No deployments found.</p>
    </div>
  {:else if loading && deployments.length === 0}
    <div class="flex flex-col items-center justify-center h-64 border-2 border-dashed rounded-2xl bg-slate-50/50 dark:bg-slate-900/30 border-slate-200 dark:border-slate-800 animate-pulse">
      ...
      <p class="text-sm text-slate-400 font-bold uppercase tracking-widest">Loading deployments...</p>
    </div>
  {/if}
</div>
