...
* **동적 IP 접속 차단**: 쿠버네티스 파드(Pod)들은 재부팅 시마다 IP가 무작위로 변경됩니다. 보안 강화 위해 특정 IP 대역만 접근을 허용해야 했으나, 동적으로 변하는 파드 IP 대역을 DB 설정 파일(`pg_hba.conf`)에 동적으로 반영해 주지 못해 접속 차단 현상이 생겼습니다.
